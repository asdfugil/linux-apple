...
static inline void s5l87xx_timer_enable(struct s5l87xx_timer *timer)
{
    pr_debug("%s...\n", __func__);
    u32 tcon;
    tcon = readl_relaxed(timer->base + TIMER_C0 + REG_CON);
    writel_relaxed(tcon | (7<<12), timer->base + TIMER_C0 + REG_CON);
    writel_relaxed(0x1000, timer->base + TIMER_C0 + REG_DATA0);
